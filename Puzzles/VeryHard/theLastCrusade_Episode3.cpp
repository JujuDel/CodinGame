    ...
    for (const pair<Coord, DIRECTION>& rock : rocks[rocks.size() - 1]) {
        if (rock.second == NONE) {
            ...
            if (room.find(rock.second) != room.end() && room.at(rock.second) != NONE) {
                ...
                if (nextCoor.x < 0 || nextCoor.x >= grid[0].size() || nextCoor.y < 0 || nextCoor.y >= grid.size()) {
...
bool interceptRocks(const vector<vector<int>>& grid,
                    const vector<vector<unordered_map<DIRECTION, DIRECTION>>>& roomsWithRotation,
                    const vector<pair<int, int>>& idsInRooms,
                    vector<vector<pair<Coord, DIRECTION>>>& rocksRound,
                    Node* n,
                    const vector<int>& idxKillingRocks)
{
    bool allGood = true;
    for (const int& idxKiller : idxKillingRocks) {
        cerr << "TRYING INTERCEPTION..." << endl;
        Node* node = n;
        bool isGood = false;
        for (int round = rocksRound.size() - 2; node && round > 0; node=node->parent, --round) {
            const pair<Coord, DIRECTION> killerRock = rocksRound[round][idxKiller];
            const pair<int, int> pairKiller = idsInRooms[abs(grid[killerRock.first.y][killerRock.first.x])];
            unordered_map<DIRECTION, DIRECTION> roomKiller = roomsWithRotation[pairKiller.first][pairKiller.second];
            
            cerr << "  On " << killerRock.first.x << " " << killerRock.first.y << endl;

            for (const pair<DIRECTION, DIRECTION>& umap : roomKiller) {
                const DIRECTION from = umap.first;
                cerr << "   from " << dirtostr(from) << endl;
                const Coord savior{ killerRock.first.x + directionX[from], killerRock.first.y + directionY[from] };
                
                for (int idxSavior = 0; idxSavior < rocksRound[round].size(); ++idxSavior) {
                    if (idxSavior == idxKiller)
                        continue;

                    if (rocksRound[round - 1][idxSavior].first == savior) {
                        cerr << "     found pot savior on " << rocksRound[round - 1][idxSavior].first << endl;
                        const pair<int, int> pairSavior = idsInRooms[abs(grid[savior.y][savior.x])];
                        for (int rotation = 0; rotation < roomsWithRotation[pairSavior.first].size(); ++rotation) {
                            unordered_map<DIRECTION, DIRECTION> room = roomsWithRotation[pairSavior.first][rotation];
                            if (room.find(rocksRound[round - 1][idxSavior].second) != room.end() && room.at(rocksRound[round - 1][idxSavior].second) == oppositeDir[from]) {
                                isGood = true;
                                cerr << "       found rotation" << endl;
                                
                                int left, right;
                                if (rotation < pairSavior.second) {
                                    right = rotation + roomsWithRotation[pairSavior.first].size() - pairSavior.second;
                                    left = pairSavior.second - rotation;
                                }
                                else {
                                    left = pairSavior.second + roomsWithRotation[pairSavior.first].size() - rotation;
                                    right = rotation - pairSavior.second;
                                }
                                int penalty = min(left, right);
                                if (penalty == left) penalty *= -1;  
                                ...
                                Node* newNode = new Node(rocksRound[round - 1][idxSavior].first, rocksRound[round - 1][idxSavior].second, node->score, node->rotation, node->parent);
                                node->parent = newNode;
                                node->rotation = (penalty > 0 ? RIGHT : LEFT);
                                n->score -= abs(penalty);
                                cerr << "INTERCEPTION FOUND!" << endl;
        ...
        rocksRound[rocksRound.size() - 1][idxKiller].second = NONE;
        allGood &= isGood;
    }
    return allGood;
}

bool checkRocks(const vector<vector<int>>& grid,
                const vector<vector<unordered_map<DIRECTION, DIRECTION>>>& roomsWithRotation,
                const vector<pair<int, int>>& idsInRooms,
                vector<vector<pair<Coord, DIRECTION>>>& rocksRound,
                Node* n)
{
    vector<int> idxKillingRocks;
    ...
    for (int idRock = 0; idRock < rocksRound[round].size(); ++idRock) {
        ...
        if (n->coord == rocksRound[round][idRock].first && idsInRooms[abs(grid[n->coord.y][n->coord.x])].first != 3) {
            ...
            for (int i = 0; node && i <= round; ++i, node=node->parent) {
                ...
                while (penalty < nbRotationRoom) {
                    ...
                    if (roomRight.find(rock.second) == roomRight.end() || (roomRight.find(rock.second) != roomRight.end() && roomRight.at(rock.second) == NONE)) {
                        if (roomRight.find(n->from) != roomRight.end()) {
                            break;
                        }
                    }
                    if (left == right)
                    ...
                    if (roomLeft.find(rock.second) == roomLeft.end() || (roomLeft.find(rock.second) != roomLeft.end() && roomLeft.at(rock.second) == NONE)) {
                        if (roomLeft.find(n->from) != roomLeft.end()) {
                            penalty *= -1;
                            break;
                        }
                    }
                ...
                if (node->parent) {
                    ...
                    //n->score -= abs(penalty);
                    rocksRound[rocksRound.size() - 1][idRock].second = NONE;
            ...
            if (!atLeastOne) {
                idxKillingRocks.push_back(idRock);
            }
        }
    }

    return interceptRocks(grid, roomsWithRotation, idsInRooms, rocksRound, n, idxKillingRocks);
}
          ...
          const vector<pair<Coord, DIRECTION>>& rocks)
{
    ...
    while (queue.size() > 0) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!checkRocks(grid, roomsWithRotation, idsInRooms, rocksRound, n)) {
                cerr << "UNAVOIDABLE DEATH..." << endl;
                continue;
            }
...
int main()
{
    ...
    while (1) {
        ...
        if (rot == NONE) {
            if (W == 14 && H == 14 && grid[7][8] != 8) {
                cout << "8 7 RIGHT" << endl;
                grid[7][8] = nextTypeValue(grid[7][8], 1);
            }
            else
                cout << "WAIT" << endl;
        }
        else {
            if (W == 14 && H == 14 && x == 8 && y == 7)
                cout << "WAIT" << endl;
            else {
